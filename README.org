...
The general idea for the use of advanced algorithms is the same as when you are using any other external resource, such as verbal advice, online forums, or material from books or websites.  In all cases, you are /not/ allowed to present anybody's work as your own work. This includes both human and machine generated output. Misrepresentation will be considered misuses in this class.  So, you must cite each and every use of any external tool, reference, or personal help. Here are some examples:
   ...
   10. You will be tested in the oral presentation. 
...
There is one project, taking up 40% of the credit.
There is one written exam, for 40% of the credit.
Assignments, for 20% of the credit.
...
2. Linearity of Expectations (5') [not done]
3. Convexity of Bayes Decisions (5') [not done]
...
Simplified notation
****  ML Estimation (10')
- $\omega_{ML} = \argmax_\omega P(x | \omega)$.
- $a^* = \argmax_a U(\omega_{ML}, a)$.
**** MAP Estimation (10')
- $\omega_{MAP} = \argmax_\omega P(\omega | x)$
- $a^* = \argmax_a U(\omega_{MAP}, a)$.
**** Bayes Estimation (10')
- $a^* = \argmax_a \sum_\omega P(\omega | x) U(\omega, a)$.
...
- Prior belief $\bel$
- Find $\mu$ maximising $\bel(\mu) P_\mu(x)$
...
\]
- $n_{T,a}$ is the number of times $a$ has been pulled after $T$ steps.
...
At time $t$:
- Each expert $i \in A$ makes probabilistic predictions $p_{i,t}(x)$ for every possible $x \in X$.
- You make a choice of some expert $a_t \in A$
- You observe $x_t \in X$
- You obtain reward $r_t = - \ln p_{a_t, t}(x)$
- You calculate the rewards of all experts, $r_{i,t} = - \ln p_{i, t}(x)$.
...
- Starting with a uniform prior on these values, calculate the
  posterior distribution after you observe 100 throws of a fair coin.
- How does this posterior compare to the one resulting from the Beta(1,1) prior?


**** Confidence bounds
Where you toss a coin of unknown bias $\theta$ 100 times. Calculate the empirical mean $\hat{\theta}$ and compare it with the true expected value $\theta$.
How often is $|\hat{\theta} - \theta| \leq \sqrt{\frac{\ln(2/\delta)}{2t}}$, where $t$ is the number of throws?
Do the calculation by running the experiment 10,000 times and counting the fraction of the time where this fails, for values of $t \in \{1, \ldots, 100\}$, and for values of $\delta \in \{0.001, 0.01, 0.1\}$. How close are the estimates?
...
**** Solving $\sqrt(x)$                                      :B_exampleblock:
:PROPERTIES:
:BEAMER_env: exampleblock
:END:
- The mapping has the property $f(x) = x$ for the solution.
- So if $x = \sqrt{x_0}$, we can write $x^2 - x_0 = 0$.
- Rewrite as $2 x^2 = x^2 + x_0$ $\Rightarrow$ $x = \frac{1}{2} \left(x + x_0/x \right)$.
- Or.. $(c+1) x^2 = c x^2 + x_0$ $\Rightarrow$ $x = \frac{1}{c+1} \left(c x + x_0/x \right)$.
