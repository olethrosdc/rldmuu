...
**** Prediction with expert advice
   ...
   - Obtain reward $r_t = \rho(a_t, y_t)$
   - Utility $U = \sum_{t=1}^T U_t$.
   - $T$ is the problem *horizon*

**** Algorithm
   - Weights $\xi_t(\mu)$ for each model $\mu$.
   - Make decision $a_t = \argmax_a \sum_\mu  \xi_{t}(\mu) \sum_y p_{t,\mu}(y) \rho(a, y)$
   - Update: $\xi_{t+1}(\mu) \propto \xi_t(\mu) p_{t,\mu}(y_t)$.
