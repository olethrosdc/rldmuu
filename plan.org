...
|------+--------------------------------------------|
|    2 | Decisions with Observations                |
|      | Bayesian Decision Rules                    |
|------+--------------------------------------------|
|    3 | Introduction to bandit problems.           |
|      | Beliefs and estimation.                    |
|------+--------------------------------------------|
|    4 | Bandit problems and MDPs.                  |
|      | Belief states, Backwards Induction.        |
|------+--------------------------------------------|
...
1. n-meteorologists vs partial information (15')
   ...
   In the n-meteorologists problem, we make a prediction about the future baed on the opinion of every model. We then make a decision and see the true outcome, and can adjust our beliefs about which model is correct. More formally, we have the following sequence:

   
   - Observe predictions $p_t= p_{t,1}, \ldots, p_{t,n}$ of all models.
   - Make decision $a_t$ according to $P_t = \xi_t p_t$.
   - Observe true outcome $y_t$
   - Measure score $s_{t,i} = f(y_t, p_{t,i})$ for each model $i$.
   - Adjust belief $\xi_{t+1} \propto \xi_t s_t$.

   In partial information problems, our observations depend on our actions. So we can only adjust our belief for those models whose predictions are related to the observations. The classical setting of this is the stochatic bandit prblem.
...
3. A simple algorithm: a simple belief and greedy actions (15')

- For every arm we pull, we can keep track of the average reward.
- This requires maintaining a /sufficient statistic/ (see chapter 4)
  $n_{t,i}$: the number of times we pulled arm $i$ until time $t$.
  $s_{t,i}$: the total reward we obtained from arm $i$ until time $t$.
- This allows us to use the mean estimate $\hat{\mu}_{t,i} = s_{t.i} / n_{t.i}$.
- So we can be greedy, and just choose the arm with the highest mean estimate.
  

4. Beta distributions as beliefs
