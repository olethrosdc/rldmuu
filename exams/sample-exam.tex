...
\\[1cm]
\begin{large}
  \textbf{Reinforcement Learning and Decision Making Under Uncertainty}
\end{large}
\\[.5cm]
Informatics\\
...
\begin{question}{Reinforcement learning}{2}
  ...
  \if \GiveSolution 1
  \begin{solution}
    Firstly, SARSA would satisfy $\lim_{t \to \infty} Q_t = Q^{\pi_{SARSA}}$, while $\lim_{t \to \infty} Q_t = Q^{*}$, unless action selection becomes less and less random. So SARSA would converge to the value of the softmax policy, while Q-learning to the value of the optimal policy. However, both would play softmax wrt the respective Q-values. For that reason one might expect that SARSA performed better sometimes.

    Any MDP will do, actually, but a good example is the windy gridworld from Sutton and Barto's book.
  \end{solution}
  \fi
\end{question}
...
\begin{question}{The gambler}{2}
  ...
  \if \GiveSolution 1
  \begin{solution}
    Because the game ends only when you either reach 100 or 0, and the reward is 1 only when you reach 100, that means the gambler's goal is to simply get 100 coins. This is not, strictly speaking, the same as trying to maximise profits.

    A simpler solution would be to have the gambler decide when to quit, with an additional 'quit' action. Then the reward after quitting could simply be an increasing function of the amount of money the gambler has at the time. (as a side-note, it should then be obvious that if this function is concave (or linear), then the gambler should just immediately quit, at least in this particular game.)
  \end{solution}
  \fi
\end{question}
