...
\begin{exercise}[15]
  ...
  \item Perform an experiment to validate your claims using the code in \verb!src/paradox.py!
  \end{itemize}
\end{exercise}
...
\begin{proof}[Exercise 2 solution]
  ...
  \[
  \dd/\dd x \left(\int_a^x f(t)\right) \dd{t} = f(x)
  \]
  to show that the derivative of $\E(U \mid a)$ with respect to $a$ is zero at $a = \mu$.
\end{proof}
\fi
\begin{exercise}[usefulness of probability and utility]
  \begin{enumerate}
