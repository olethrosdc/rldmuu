...
\begin{exercise} If you have not done it already, implement the Backwards Induction algorithm in \verb!src/BackwardsInduction.py! that takes an input \textbf{any MDP} (defined in \verb|src/MDP.py|) and outputs the optimal policy, and its state and state-action value. Do the same for Value Iteration and Policy Iteration.
  
\end{exercise}

\begin{exercise}[In-class, continue at home if needed.]
  \begin{enumerate}  
  \item Choose one of the problems below that can be modelled as a Markov decision process with a finite or infinite horizon, including the state space, action space, reward function and transition distribution. 
  \item Implement the MDP as an instance of the \verb|MDP| class.
  \item Calculate the optimal policy and value function for your problem, and interpret the results. What does the value of the different states mean? Is the optimal policy what you expected it to be? How do the different algorithms compare in terms of computation time and convergence speed?
  \item Discuss your results in class.
  \end{enumerate}
\end{exercise}
